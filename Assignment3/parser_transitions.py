...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unparsed_sentences) > 0:
        ...
        # use model to predict the next transition for these sentences
        transitions = model.predict(minibatch)
        # perform parse step on each sentence in minibatch using the predicted next transition
        for i, pp in enumerate(minibatch):
            pp.parse_step(transitions[i])
        
            # check which sentences in minibatch have been fully parsed
            if len(pp.buffer)==0 and len(pp.stack)==1:
                # remove the index of fully parsed sentence from unparsed sentences
                ix = minibatch_indices[i]
                unparsed_sentences.remove(ix)
                # copy the dependencies
                dependencies[ix] = pp.dependencies
